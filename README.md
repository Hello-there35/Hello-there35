<h1> Hi there I'm Sean Dubbelman👋</h1>

![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&size=24&duration=2000&pause=500&color=1233123&width=435&lines=I+am+a+web+Developer)

![Hero image](https://github.com/user-attachments/assets/1ee6297e-d6e8-4ce0-9b2b-654ab5a310ea)

<h4>Where am I at the moment?🤔 </h4>
<p>I am currently Studying at Open Window in Centurion. I am in my Final year of Study and I am
looking to get some experience and join in internships and Job shadowing to prepare for my
first Web Dev Job.</p>

<h2>Skills</h2>

<h4>Coding Skills</h4>

[![My coding skills](https://skillicons.dev/icons?i=js,html,jquery,react,py,php,dotnet,typescript,cs,electron)](https://skillicons.dev)

<h4>Styling Skills</h4>

[![My styling skills](https://skillicons.dev/icons?i=css,bootstrap,materialui,sass)](https://skillicons.dev)

<h4>Database and deployment Skills</h4>

[![Database skills](https://skillicons.dev/icons?i=firebase,postgres,supabase,mongodb,mysql,azure,docker,heroku)](https://skillicons.dev)

<h4>Other Skills</h4>

[![Other skills](https://skillicons.dev/icons?i=wordpress,webflow,git,figma)](https://skillicons.dev)


<h2>Github stats</h2>

![Hello-there35 GitHub stats](https://github-readme-stats.vercel.app/api?username=Hello-there35&show_icons=true&theme=radical)
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=Hello-there35&layout=compact&theme=radical)


<h1 align="center">Projects</h1>
<p> Here are the projects I have made and worked on through learning new technologies.I have gone from using plain javascript to Mobile development and AI. Through these 
projects I have learnt many new ways to plan and execute how my project will run and how it will has to used according to the users.</p>
<br>



<p align="center">
<img width="150" height="200" alt="QuaestioCraftor-logo" src="https://github.com/user-attachments/assets/c1fc5cb6-9852-4e12-b3fc-6bf87d436f2f" />
</p>
<p align="center">
<h2 align="center">📱 QuaestioCraftor 📱</h2>   
</p>

**🔗 Repository:** [View on GitHub](https://github.com/Sean-D-231121/QuaestioCraftor-react-native-app)  
**🧠 Backend:** [View Backend Repo](https://github.com/Sean-D-231121/QuaestioCraftor-backend)

#### 📸 **Preview / Screenshots**  
<p align="center">
  <img width="80%" alt="QuaestioCraftor-mockup" src="https://github.com/user-attachments/assets/79131c17-d6a8-47c5-bdd6-4cdb1b9788c9" border-radius="10px"/>
</p>


#### 📝 **Description**  
**QuaestioCraftor** is a **React Native mobile app** that allows users to create, share, and complete quizzes.  
It was designed to make learning interactive and social by turning educational quizzes into engaging, creative challenges.

#### 🎯 **Purpose / Motivation**  
This project explores **gamified learning and content creation**.  
The goal was to make education more enjoyable and approachable not just for students, but for anyone who wants to learn or test their knowledge through play.  
It was also a deep dive into **mobile app development and backend integration** using Supabase.

<p align="center">
<img width="100" height="150" alt="QuietSpacer-logo" src="https://github.com/user-attachments/assets/35a3409e-a6c8-4c48-80c6-18ad3d48d886" />
</p>

<h2 align="center">🧘 QuietSpacer 🧘 </h2>

**🔗 Repository:** [View on GitHub](https://github.com/Sean-D-231121/QuietSpacer-App-DV300-Semester-2-Term-3)

#### 📸 Preview / Screenshots  
<p align="center">
<img width="80%" alt="QuietSpacer-mockup" src="https://github.com/user-attachments/assets/6916b756-0623-4f15-926a-0ddd06389d95" />
</p>

#### 📝 Description  
**QuietSpacer** is a mobile geolocation-mindfulness application designed to help users reduce stress and increase calmness by discovering and using peaceful places. The app lets users explore quiet spots, track moods, bookmark favourites, and contribute reviews.

#### 🎯 Purpose / Motivation  
This app stems from the need to combine **well-being**, **location technology**, and **user-centred design**. The goal was to create a tool that encourages mindful breaks and supports mental health by making calm spaces easier to find and use.



<p align="center">
<img width="100" height="200" alt="Aureume-Elegance-logo" src="https://github.com/user-attachments/assets/755387c6-780f-4b9c-9e8d-1bf2b04f5d36" />
</p>

<h2 align="center">💫 Aureum Elegance 💫</h2>

**🔗 Repository:** [View on GitHub](https://github.com/Tsebo200/Aureum-Elegance-Frontend)


#### 📸 Preview / Screenshots  
<p align="center">
<img width="80%" alt="Aureume-Elegance-mockup" src="https://github.com/user-attachments/assets/43348044-5c15-4c1c-8420-c6b5a575cbcf" />
</p>

#### 📝 Description  
**Aureum Elegance** is a full-stack warehouse management system tailored specifically for the perfume industry. It streamlines inventory tracking, order processing, and analytics by leveraging modern web technologies and a user-friendly interface.

#### 🎯 Purpose / Motivation  
This project was built with the goal to help a client who has been tirelessly working on bookkeeping through ordinary means but it has been hard to keep track of orders and how much is produced each month. She wanted an application that can be downloaded and used by their organisation to easily tracks orders and production of fragrances for better bookkeeping.





<h1>Some info about me🤔</h1>

- 🌱 I’m currently learning Supabase
- 👯 I’m looking to collaborate on anything Web-related as I would like to learn more about the field. Besides just making apps.
- 🤔 I’m looking for help with learning SEO
- 📫 How to reach me: sean.tutors01@gmail.com
- ⚡ Fun fact: I am also a UX/UI Designer
